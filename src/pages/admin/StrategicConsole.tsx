...
export default function StrategicConsole() {
  ...
  const ensureDatabaseSeeded = async () => {
    // 100% genuine content only - no artificial items are written on the fly.
  };
  ...
  useEffect(() => {
    const unsubTasks = onSnapshot(collection(db, 'tasks'), (taskSnap) => {
      const list = taskSnap.docs.map(d => ({ id: d.id, ...d.data() as any }));
      setTasksList(list);
    });
    return unsubTasks;
  }, []);
  ...
  useEffect(() => {
    const appInstallsCount = tasksList.filter((t: any) => t.status === 'active' && /install|app|download/i.test(t.title + ' ' + t.description)).length;
    const socialSharesCount = tasksList.filter((t: any) => t.status === 'active' && /share|tweet|retweet|social|follow|comment/i.test(t.title + ' ' + t.description)).length;
    const videoReviewsCount = tasksList.filter((t: any) => t.status === 'active' && /video|review|youtube|watch/i.test(t.title + ' ' + t.description)).length;
    const affiliateSalesCount = tasksList.filter((t: any) => t.status === 'active' && /affiliate|sale|lead|purchase/i.test(t.title + ' ' + t.description)).length;
    const dataLabelingCount = tasksList.filter((t: any) => t.status === 'active' && /label|data|annotate/i.test(t.title + ' ' + t.description)).length;
